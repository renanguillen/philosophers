...
static void	assign_forks(t_philosopher *philo)
{
	if (philo->id % 2)
	{
		philo->fork[0] = (philo->id + 1) % philo->simulation->nb_philos;
		philo->fork[1] = philo->id;
	}
	else
	{
		philo->fork[0] = philo->id;
		philo->fork[1] = (philo->id + 1) % philo->simulation->nb_philos;
	}
	return ;
}
...
static int	init_philosophers(t_simulation *simulation)
{
	...
	while (i < simulation->nb_philos)
	{
		...
		if (pthread_mutex_init(&simulation->philos[i]->meal_time_lock, NULL))
